# E2E 测试

端到端（End-to-End）测试，使用 Playwright 框架。

---

## 🚀 快速开始

### 方式 1：一键运行（推荐）⭐

```bash
# 在 frontend/web 目录
pnpm e2e:all
```

一条命令完成：启动 Docker 环境 → 运行测试 → 清理环境

### 方式 2：手动控制

#### 1. 安装 Playwright 浏览器（首次）

```bash
pnpm playwright:install
```

#### 2. 启动 E2E 环境（Docker）

```bash
# 在 frontend/web 目录
pnpm e2e:setup

# 或在项目根目录
./docker/e2e/start.sh
```

这会启动：
- ✅ Postgres 数据库（localhost:5433）
- ✅ Redis 缓存（localhost:6381）
- ✅ Go 后端服务器（http://localhost:8081）
- ✅ Node.js 后端服务器（http://localhost:8082）
- ✅ 预置测试用户和数据

#### 3. 运行 E2E 测试

```bash
# 在 frontend/web 目录
# 测试 Go 后端（默认）
pnpm e2e              # 运行所有测试
pnpm e2e:ui           # UI 模式（推荐，可视化调试）⭐
pnpm e2e:headed       # 有头模式（显示浏览器）
pnpm e2e:debug        # 调试模式

# 测试 Node.js 后端
pnpm e2e:nodejs       # 运行所有测试
pnpm e2e:nodejs:ui    # UI 模式（推荐）⭐
```

#### 4. 停止 E2E 环境

```bash
# 停止但保留数据
pnpm e2e:teardown

# 停止并清理所有数据
pnpm e2e:clean
```

---

## 📁 目录结构

```
e2e/
├── auth/                       # 认证流程测试
│   ├── login.spec.ts           # 登录测试
│   └── register.spec.ts        # 注册测试
├── task/                       # 任务管理测试
│   ├── create-task.spec.ts     # 创建任务
│   ├── task-operations.spec.ts # 任务操作（编辑、删除、完成）
│   └── task-flow.spec.ts       # 完整流程测试
├── fixtures/                   # 测试数据
│   └── test-data.ts
└── helpers/                    # 辅助函数
    ├── auth-helpers.ts
    └── task-helpers.ts
```

---

## 🧪 测试覆盖

### Auth Flow（认证流程）
- ✅ 成功登录
- ✅ 错误凭据登录
- ✅ 表单验证
- ✅ 登出
- ✅ 成功注册
- ✅ 邮箱已存在
- ✅ 密码验证
- ✅ 页面导航

### Task Flow（任务流程）
- ✅ 创建基本任务
- ✅ 创建高优先级任务
- ✅ 创建带标签任务
- ✅ 编辑任务
- ✅ 完成任务
- ✅ 删除任务
- ✅ 任务筛选
- ✅ 完整生命周期
- ✅ 路由保护

**总计**: 17+ 个 E2E 测试用例

---

## 🔧 配置

### playwright.config.ts

- **测试目录**: `./e2e`
- **超时时间**: 30 秒
- **失败重试**: CI 环境 2 次
- **浏览器**: Chromium（默认）
- **截图**: 仅失败时
- **视频**: 仅失败时保留

### 环境变量

#### 选择后端实现

E2E 测试支持两种后端实现：

1. **Go 后端**（默认）：`http://localhost:8081`
2. **Node.js 后端**：`http://localhost:8082`

#### 方式 1：使用 npm 脚本（推荐）

```bash
# 测试 Go 后端（默认）
pnpm e2e              # 命令行模式
pnpm e2e:ui           # UI 模式

# 测试 Node.js 后端
pnpm e2e:nodejs       # 命令行模式
pnpm e2e:nodejs:ui    # UI 模式

# 一键运行（包含环境启动和清理）
pnpm e2e:all          # Go 后端
pnpm e2e:nodejs:all   # Node.js 后端
```

#### 方式 2：使用环境变量

```bash
# 测试 Go 后端（默认）
E2E_BACKEND_URL=http://localhost:8081 pnpm e2e

# 测试 Node.js 后端
E2E_BACKEND_URL=http://localhost:8082 pnpm e2e
```

#### 方式 3：创建 `.env.e2e` 文件（可选）

在 `frontend/web/` 目录下创建 `.env.e2e` 文件：

```bash
# 选择后端（二选一）
E2E_BACKEND_URL=http://localhost:8081  # Go 后端
# E2E_BACKEND_URL=http://localhost:8082  # Node.js 后端

# 前端地址（通常不需要修改）
E2E_BASE_URL=http://localhost:5173
```

---

## 📝 测试最佳实践

### 1. 使用辅助函数

```typescript
import { loginAsTestUser } from '../helpers/auth-helpers'

test('my test', async ({ page }) => {
  await loginAsTestUser(page)  // 使用辅助函数登录
  // ...
})
```

### 2. 使用测试数据

```typescript
import { testTasks } from '../fixtures/test-data'

await page.fill('input', testTasks.basic.title)
```

### 3. 使用明确的选择器

```typescript
// ✅ 好
await page.click('button[data-testid="create-task"]')

// ❌ 差
await page.click('button')
```

### 4. 等待元素

```typescript
// ✅ 自动等待
await expect(page.locator('text=Success')).toBeVisible()

// ❌ 固定延迟
await page.waitForTimeout(3000)
```

---

## 🐛 调试技巧

### 1. 使用 UI 模式

```bash
pnpm e2e:ui
```

可视化界面，可以：
- 看到每个步骤的执行
- 查看截图
- 查看网络请求
- 逐步调试

### 2. 使用调试模式

```bash
pnpm e2e:debug
```

### 3. 查看测试报告

```bash
pnpm exec playwright show-report
```

### 4. 截图和视频

失败的测试会自动保存：
- 截图：`test-results/`
- 视频：`test-results/`
- HTML 报告：`playwright-report/`

---

## ⚠️ 注意事项

### 测试数据

E2E 测试使用 Docker 隔离环境：

1. **测试用户**：自动创建 `e2e-test@example.com`（密码：`Test123456!`）
2. **测试数据库**：独立的 Postgres 实例（端口 5433）
3. **数据清理**：使用 `pnpm e2e:clean` 完全清理

### 并发问题

- CI 环境设置为串行执行（`workers: 1`）
- 避免测试间数据冲突

---

## 📊 CI/CD

E2E 测试在 GitHub Actions 中自动运行：

- ✅ 自动启动 Postgres 数据库
- ✅ 自动启动后端服务器
- ✅ 自动启动前端开发服务器
- ✅ 运行 E2E 测试
- ✅ 上传测试报告（失败时）
- ✅ 上传视频（失败时）

查看配置：`.github/workflows/frontend-e2e.yml`

---

## 🔗 相关资源

- [Playwright 文档](https://playwright.dev/)
- [测试方案](../../docs/FRONTEND_E2E_PLAN.md)
- [前端 README](../README.md)

---

**维护者**: AI Assistant  
**最后更新**: 2025-11-27

