...
| 测试场景易碎、选择器不稳 | **Playwright-friendly 设计 + data-test-id 规范 + Docker E2E 环境** |
...
### 🧪 测试友好设计（Playwright / Vitest Ready）

在 AI 辅助开发场景下，代码可测试性直接决定 AI 生成代码的可验证性。本项目专门为自动化测试做了优化：

- **前端 Playwright-friendly**：所有交互元素强制 `data-test-id` 命名规范，避免依赖样式类；页面/路由状态稳定，减少 flaky。
- **隔离的 E2E 环境**：Docker 启动独立的 Postgres(:5433) + Backend(:8081) 环境，与开发端口隔离，可并行运行；`pnpm e2e:all` 一键启动/测试/清理。
- **稳定测试数据**：预置种子数据和固定测试账号；Playwright fixtures/helpers 复用，避免魔法字符串。
- **快速单测反馈**：Vitest + happy-dom + 线程池并发；`pnpm test:watch` 秒级反馈，`pnpm test:coverage` 输出覆盖率。
- **目录即规则**：Feature 内 `__tests__` 贴近业务代码（hooks/stores/api/components），便于 AI/人类快速定位。
- **CI 友好**：前端 E2E 三层缓存（pnpm store / Playwright 浏览器 / Docker layer），后端 `./backend/scripts/test_all.sh` 支持覆盖率输出，降低流水线耗时。

> 目标：让“AI 生成 → 自动化校验 → 快速修正”形成闭环，减少人工回归成本。
...
| **Testing** | 单元 + 集成 + E2E | `./backend/scripts/test_all.sh` / `pnpm test` / `pnpm e2e:all` |
...
- ✅ Docker 一键启动
- ✅ 完整的开发脚本
- ✅ Playwright-friendly 前端（data-test-id 规范、Docker E2E 环境）
- ✅ Vitest + happy-dom 单测基建（线程池并发、秒级反馈）
...
<small>data-test-id 规范 + Docker E2E 隔离</small>
</td>
<td align="center" width="25%">
<h3>✅ 代码质量</h3>
<h2>100%</h2>
