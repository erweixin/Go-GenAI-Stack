...
```
go-genai-stack/
├── backend/              # 后端（Go + Hertz + Eino）
├── frontend/             # 前端（React + TypeScript + shadcn/ui）
...
### 5.2 后端结构（Vibe Coding Friendly DDD 架构）

> 💡 **重要说明**：这不是传统的 DDD 架构！  
> **Vibe Coding Friendly DDD** 是我们专为 AI 辅助编程时代设计的架构变体。
...
### 5.3 前端结构（Vibe Coding Friendly React）
...
│   │   │   │                                  # - 下拉选择模型（GPT-4、Claude 等）
│   │   │   │                                  # - 显示模型价格和速度
│   │   │   │
│   │   │   └── README.md                      # 聊天组件使用说明
│   │   │
│   │   ├── structured/                        # 结构化输出组件
│   │   │   ├── StructuredForm.tsx             # 结构化输出表单
│   │   │   │                                  # - 输入提示词
│   │   │   │                                  # - 选择 Schema
│   │   │   │
│   │   │   ├── SchemaSelector.tsx             # Schema 选择器
│   │   │   ├── JsonViewer.tsx                 # JSON 结果展示
│   │   │   └── README.md
│   │   │
│   │   ├── monitor/                           # 监控 Dashboard 组件
│   │   │   ├── MetricsCard.tsx                # 指标卡片（总请求数、成功率等）
│   │   │   ├── LatencyChart.tsx               # 延迟趋势图（Chart.js）
│   │   │   ├── CostChart.tsx                  # 成本趋势图
│   │   │   ├── ModelUsageChart.tsx            # 模型使用分布（饼图）
│   │   │   └── README.md
│   │   │
│   │   └── layout/                            # 布局组件
│   │       ├── Header.tsx                     # 页面头部
...
│   │
│   ├── pages/                                 # 页面（路由级组件）
│   │   ├── ChatPage.tsx                       # /chat - 聊天页面
│   │   ├── StructuredPage.tsx                 # /structured - 结构化输出页面
│   │   ├── MonitorPage.tsx                    # /monitor - 监控页面
│   │   ├── SettingsPage.tsx                   # /settings - 设置页面
│   │   └── README.md                          # 页面路由说明
│   │
...
│   │   │                                      # - 响应拦截器（错误处理）
│   │   │
│   │   └── endpoints/                         # API 端点（按功能分类）
│   │       ├── chat.ts                        # 聊天 API
│   │       │                                  # - POST /api/chat
│   │       │                                  # - POST /api/chat/stream
│   │       │
│   │       ├── structured.ts                  # 结构化输出 API
│   │       ├── monitor.ts                     # 监控 API
│   │       └── auth.ts                        # 认证 API
│   │
│   ├── utils/                                 # 工具函数
...
```mermaid
graph LR
    A[开发者修改 usecases.yaml] --> B[触发 AI Codegen]
    B --> C[AI 读取显式知识文件]
    C --> D[AI 生成 handler + tests]
    D --> E[CI 运行测试]
    E --> F{测试通过?}
    F -->|是| G[人工 Review]
    F -->|否| D
...
2. **集成 Hertz**：实现基础 HTTP 服务和中间件
3. **集成 Eino**：实现基础的 LLM 调用（OpenAI）
4. **Docker Compose**：配置 PostgreSQL + Redis 环境

#### 第 2-4 周：核心功能
5. **聊天功能**：实现多轮对话、流式响应、历史持久化
6. **结构化输出**：实现 JSON Schema 生成、验证、重试
7. **前端基础**：React + shadcn/ui，实现聊天界面
